...
23 份互動式 HTML 檢驗教學 deck —— **15 章 Nielsen《食品分析》** ＋ **8 項衛福部食藥署公告／建議檢驗方法**，各為 3 小時課程、淺色學術風、含 **6 個嵌入式形成性評量小遊戲**（拖放配對、選擇題、排序、計算、可排序比較表、互動圖表）。另附 **1 份機器學習 × 化學計量學延伸 deck**（五種分類器以茶葉多重元素判別產地，Python × Orange 雙路線，深色主題、手機友善）。全部單一 HTML、可離線、跨裝置（手機／平板／桌機）。

🔗 **線上入口（九宮格）**：<https://tai-shengyeh.github.io/food-analysis-decks/>

---

## 一、Nielsen《食品分析》章節（15 章）
...
**合計：23 份檢驗 deck ＋ 1 份機器學習延伸 = 24 份 · 約 619 頁 · 約 139 個小遊戲與互動圖表。**

> 第 8 項（茶葉中多重元素 TFDAF0032）為深色主題的進階變體：以 ICP-MS 14 元素指紋＋**真實 PCA／LDA 互動**（可切換元素子集即時重算正確率）示範產地鑑別，依蔡承祥等 2021《台灣農業研究》研究編寫。

## 三、機器學習 × 化學計量學（延伸）

接續茶葉多重元素方法：14 元素指紋拿到後，如何讓電腦自動判別產地？這份 deck 把 5 種分類器並排教學，每種都給「一句話直覺 ＋ Python(scikit-learn) 程式 ＋ Orange Data Mining 積木」兩條實作路線。深色主題、**手機友善**（可左右滑動翻頁）。

| deck | 模型 | 雙實作路線 | 線上連結 |
|---|---|---|---|
| 五種機器學習分類法 | LDA · Ridge · Random Forest · Boosting · SVM | Python(scikit-learn) × Orange Data Mining | [開啟 →](https://tai-shengyeh.github.io/food-analysis-decks/ml_five_models_soil/) |

> 真實留一法（LOO-CV）結果：Ridge／SVM 100%、LDA／RF 95%、Boosting 85%——小資料上集成樹過擬合、線性模型反而最穩的反直覺教學點。附 `dataset/`（含 `tea_five_models_demo.py`）可一鍵重現。
