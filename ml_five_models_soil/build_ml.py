...
add(ACT, dc.kt("路線 B · 寫幾行程式", "Python：<span class='hi'>一個迴圈</span>比完五模型") +
  ...
  '<li>執行：<b>python tea_five_models_demo.py</b></li>'
  '<li><a href="dataset/" target="_blank" rel="noopener" style="color:var(--accent);font-weight:800">📂 開啟資料集頁（下載＋說明）→</a></li></ul></div>' +
  '</div></div>')
...
add(ACT, dc.kt("動手做 · 兩條路都跑一次", "從<span class='hi'>看懂</span>到<span class='hi'>跑出來</span>") +
  ...
  '<li>下載資料：<a href="dataset/" target="_blank" rel="noopener" style="color:#7be39a;font-weight:800">📂 dataset 資料集頁 →</a>（含每個檔下載＋整包 ZIP）</li>'
  ...
  '<li>看終端機印出 5 模型的 accuracy／precision／recall</li></ul></div>' +
